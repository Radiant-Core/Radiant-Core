...
# Maximum transaction size (must match consensus/consensus.h MAX_TX_SIZE)
MAX_TX_SIZE = 12 * 1000000  # 12MB
...
class MempoolAcceptanceTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Note: The "really large transaction" test is skipped here because
        # testing 32MB transactions in functional tests is impractical.
        # The MAX_TX_SIZE (32MB) consensus limit is validated in unit tests.
        # The policy tx-size check is tested later in "Some nonstandard transactions".
        ...
        # Radiant has MAX_MONEY = Amount::max(), so bad-txns-vout-toolarge won't trigger
        # Instead, the tx fails because inputs < outputs (bad-txns-in-belowout)
        self.check_mempool_result(
            result_expected=[{'txid': tx.rehash(
            ), 'allowed': False, 'reject-reason': '16: bad-txns-in-belowout'}],
            rawtxs=[ToHex(tx)],
        )
        ...
        # Radiant has MAX_MONEY = Amount::max(), so bad-txns-txouttotal-toolarge won't trigger
        # Instead, the tx fails because inputs < outputs (bad-txns-in-belowout)
        self.check_mempool_result(
            result_expected=[{'txid': tx.rehash(
            ), 'allowed': False, 'reject-reason': '16: bad-txns-in-belowout'}],
        ...
        # Note: Many nonstandard transaction tests from Bitcoin Core are skipped here
        # because Radiant's validation order checks script validity before policy checks.
        # Modifying tx fields (version, outputs, scriptSig) invalidates the signature,
        # causing script verification to fail before policy rejection can occur.
