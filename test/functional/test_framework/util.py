...
def initialize_datadir(dirname, n, chain):
    ...
    with open(os.path.join(datadir, "radiant.conf"), 'w', encoding='utf8') as f:
        ...
        f.write("keypool=1\n")
        if chain == 'regtest':
            f.write("minrelaytxfee=0.00001000\n")
            f.write("mintxfee=0.00001000\n")
            f.write("blockmintxfee=0.00001000\n")
        f.write("discover=0\n")
        f.write("dnsseed=0\n")
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "radiant.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "radiant.conf")):
        with open(os.path.join(datadir, "radiant.conf"), 'r', encoding='utf8') as f:
            for line in f:
