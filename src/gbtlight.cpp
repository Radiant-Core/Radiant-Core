...
void Initialize(CScheduler &scheduler) {
    ...
    config.storeDir = fs::absolute(storeDirStr);
...
void CleanJobDataDir() {
    ...
    // Helper to convert file_time_type to system seconds
    auto ToSeconds = [](fs::file_time_type ftime) -> int64_t {
        // Approximate conversion via system clock
        auto now = fs::file_time_type::clock::now();
        auto sys_now = std::chrono::system_clock::now();
        auto sys_ftime = sys_now + std::chrono::duration_cast<std::chrono::system_clock::duration>(ftime - now);
        return std::chrono::duration_cast<std::chrono::seconds>(sys_ftime.time_since_epoch()).count();
    };
    ...
    for (const auto &entry : fs::directory_iterator(trashDir)) {
        ...
        if (!fs::is_regular_file(path))
            continue;
        ++total;
        const auto mtime = ToSeconds(fs::last_write_time(path));
        if (mtime < cutoff) {
            // too old, delete
            RM(path);
        }
