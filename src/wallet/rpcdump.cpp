    ...
    try {
        ...
        if (pubKeys.size() && keys.size() == 0) {
            ...
            if (keys.size()) {
                ...
                CPubKey pubKeyDerived = key.GetPubKey();
                assert(key.VerifyPubKey(pubKeyDerived));

                CTxDestination pubkey_dest_derived = pubKeyDerived.GetID();

                // Consistency check.
                if (!(pubkey_dest_derived == dest)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                                       "Consistency check failed");
                }

                CKeyID vchAddress = pubKeyDerived.GetID();
                pwallet->MarkDirty();
                ...
                if (!pwallet->AddKeyPubKey(key, pubKeyDerived)) {
                    throw JSONRPCError(RPC_WALLET_ERROR,
                                       "Error adding key to wallet");
                }
