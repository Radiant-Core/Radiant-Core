...
bool HandleAuthInfo(HTTPRequest *req)
{
    ...
    UniValue::Object obj;
    obj.emplace_back("mode", UniValue(g_webui_use_password ? "password" : "cookie"));
...
bool HandleStaticFile(HTTPRequest *req, const std::string &path)
{
    ...
    if (!assets_dir.empty()) {
        const fs::path disk_path = fs::path(assets_dir) / fs::path(file_path);
    ...
    // 2. Embedded assets compiled into the binary.
    ...
    if (it != assets.end()) {
        ...
        req->WriteReply(HTTP_OK, std::string(it->second.content));
        return true;
    }
