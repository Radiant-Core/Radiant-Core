...
namespace {
/// Internal SHA-512/256 implementation.
namespace sha512_256 {
    ...
    /** Perform one SHA-512/256 transformation, processing a 128-byte chunk. */
...
CSHA512_256::CSHA512_256() : bytes(0) {
    sha512_256::Initialize(s);
}

CSHA512_256 &CSHA512_256::Write(const uint8_t *data, size_t len) {
    ...
    if (bufsize && bufsize + len >= 128) {
        ...
        bytes += 128 - bufsize;
        data += 128 - bufsize;
        sha512_256::Transform(s, buf);
        bufsize = 0;
    }
    while (end - data >= 128) {
        // Process full chunks directly from the source.
        sha512_256::Transform(s, data);
        data += 128;
        bytes += 128;
    }
    if (end > data) {
...
void CSHA512_256::Finalize(uint8_t hash[OUTPUT_SIZE]) {
    ...
    // sha512/256 is the same as sha512 except we do not fill the last 32 bytes (and we had different starting iv)
