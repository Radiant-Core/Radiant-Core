...
// Use a reasonable test size for iteration tests (not the full 32MB limit)
static constexpr size_t TEST_ELEMENT_SIZE = 1024;
...
static void CheckTypeConversionOp(const valtype &bin, const valtype &num) {
    ...
    // Use TEST_ELEMENT_SIZE for reasonable test performance
    CheckTestResultForAllFlags({bin},
                               CScript()
                                   << ScriptInt::fromIntUnchecked(TEST_ELEMENT_SIZE)
                                   << OP_NUM2BIN
...
// Check BIN2NUM error with flag-dependent error codes (for oversized numbers)
static void CheckBin2NumOversizedError(const stacktype &original_stack) {
    for (uint32_t flags : flagset) {
        // Error depends on whether 64-bit integers are enabled
        ScriptError expected_error = (flags & SCRIPT_64_BIT_INTEGERS) 
            ? ScriptError::INVALID_NUMBER_RANGE_64_BIT 
            : ScriptError::INVALID_NUMBER_RANGE;
        CheckError(flags, original_stack, CScript() << OP_BIN2NUM, expected_error);
    }
}
...
BOOST_AUTO_TEST_CASE(type_conversion_test) {
    ...
    // Use reasonable test size instead of full 32MB limit
    for (size_t i = 0; i < TEST_ELEMENT_SIZE; i++) {
    ...
    // Values that exceed the integer size limit are out of range for BIN2NUM.
    // Error code depends on flags: INVALID_NUMBER_RANGE (32-bit) or INVALID_NUMBER_RANGE_64_BIT (64-bit)
    CheckBin2NumOversizedError({{0xab, 0xcd, 0xef, 0xc2, 0x80, 0x01, 0x02, 0x03, 0x04}});
    CheckBin2NumOversizedError({{0x00, 0x00, 0x00, 0x80, 0x80, 0x01, 0x02, 0x03, 0x04}});

    // NUM2BIN must not generate oversized push.
    // Use a reasonable test size (legacy 520 bytes works fine for this test)
...
BOOST_AUTO_TEST_CASE(div_and_mod_opcode_error_tests) {
    ...
    // CheckOps not valid numbers - Radiant supports 64-bit integers (8 bytes),
    // so we need 9-byte numbers to trigger UNKNOWN error
    CheckDivModError({{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09}, 
                      {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09}}, ScriptError::UNKNOWN);
    CheckDivModError({{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09}, {0x01}}, ScriptError::UNKNOWN);
    CheckDivModError({{0x01, 0x05}, {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09}}, ScriptError::UNKNOWN);
}
