...
unsigned int GetNextClassicWorkRequired(const CBlockIndex *pindexLast, const CBlockHeader *pblock,
                                 const Consensus::Params &params) {
    ...
    return CalculateNextClassicWorkRequired(pindexLast, pindexFirst->GetBlockTime(), params);
}

unsigned int CalculateNextClassicWorkRequired(const CBlockIndex *pindexLast, int64_t nFirstBlockTime,
                                       const Consensus::Params &params) {
    if (params.fPowNoRetargeting) {
        return pindexLast->nBits;
    }
    ...
    } else if (nActualTimespan > params.nPowTargetTimespan * 4) {
        nActualTimespan = params.nPowTargetTimespan * 4;
    } 
    // Retarget
