...
UniValue generateBlocks(const Config &config,
    ...
    bool checkValidity = gArgs.GetBoolArg("-gbtcheckvalidity", DEFAULT_GBT_CHECK_VALIDITY);
    if (config.GetChainParams().MineBlocksOnDemand() && !gArgs.IsArgSet("-gbtcheckvalidity")) {
        checkValidity = false;
    }
    ...
    while (nHeight < nHeightEnd && !ShutdownRequested()) {
        ...
        std::unique_ptr<CBlockTemplate> pblocktemplate(
            BlockAssembler(config, g_mempool)
                .CreateNewBlock(coinbaseScript->reserveScript, maxGBTTimeSecs, checkValidity));
