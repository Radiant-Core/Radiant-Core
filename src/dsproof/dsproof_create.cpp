...
void hashTx(DoubleSpendProof::Spender &spender, const CTransaction &tx, size_t inputIndex)
{
    ...
    if ((hashType & 0x1f) != SIGHASH_SINGLE && (hashType & 0x1f) != SIGHASH_NONE) {
        ...
        spender.hashOutputHashes = GetHashOutputHashes(tx);
    } else if ((hashType & 0x1f) == SIGHASH_SINGLE && inputIndex < tx.vout.size()) {
        // Hash only the output at inputIndex
        {
            CHashWriter ss(SER_GETHASH, 0);
        ...
        }
        {
            ...
            writeOutputDataSummaryVector(ss, tx.vout[inputIndex].scriptPubKey, tx.vout[inputIndex].nValue, zeroRefHash);
            spender.hashOutputHashes = ss.GetHash();
        }
    }
}
} // namespace
