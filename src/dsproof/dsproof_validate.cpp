...
public:
    ...
    bool CheckSig(const std::vector<uint8_t> &vchSigIn, const std::vector<uint8_t> &vchPubKey, const CScript &scriptCode, uint32_t /*flags*/) const override {
        ...
        ss << m_amount << m_spender.outSequence;
        ss << m_spender.hashOutputHashes;
...
auto DoubleSpendProof::validate(const CTxMemPool &mempool, CTransactionRef spendingTx) const -> Validity
{
    ...
    //
    // Important: This validation may occur when the prevout is currently being
    // spent by the mempool (e.g. when rescuing/claiming an orphan DSProof after
    // the spending transaction arrives). In that case, CCoinsViewMemPool may
    // treat the coin as unavailable due to the mempool spend.
    //
    // Prefer querying pcoinsTip directly (confirmed UTXO set ignores mempool
    // spends), and fall back to CCoinsViewMemPool only if needed (e.g. prevout
    // created in the mempool).
    Coin coin;
    if (!pcoinsTip->GetCoin(outPoint(), coin)) {
        const CCoinsViewMemPool view(pcoinsTip.get(), mempool); // checks both mempool coins and confirmed coins
    ...
    if ( ! VerifyScript(inScript, prevOutScript, flags, checker1, metrics, context, &error)) {
    ...
    if ( ! VerifyScript(inScript, prevOutScript, flags, checker2, metrics, context, &error)) {
