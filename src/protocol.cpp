...
bool CMessageHeader::IsOversized(const Config &config) const {
    // TX messages get a larger limit (16MB) to accommodate large transactions
    if (GetCommand() == NetMsgType::TX) {
        if (nMessageSize > MAX_TX_MESSAGE_LENGTH) {
            return true;
        }
        return false;
    }
    ...
    // All other messages are limited to MAX_PROTOCOL_MESSAGE_LENGTH (2MB)
    if (nMessageSize > MAX_PROTOCOL_MESSAGE_LENGTH) {
        return true;
    }
