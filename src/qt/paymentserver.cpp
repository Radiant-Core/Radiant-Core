...
void PaymentServer::ipcParseCommandLine(interfaces::Node &node, int argc,
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (itemNetwork == nullptr) {
            ...
            qWarning() << "PaymentServer::ipcSendCommandLine: URI is invalid: "
                       << arg;
            continue;
        }

        if (chosenNetwork && chosenNetwork != itemNetwork) {
            qWarning() << "PaymentServer::ipcSendCommandLine: URI from network "
...
PaymentServer::PaymentServer(QObject *parent, bool startLocalServer)
    ...
    if (startLocalServer) {
        ...
        if (!uriServer->listen(name)) {
            ...
            // message()" here
            QMessageBox::critical(nullptr, tr("URI handler error"),
...
void PaymentServer::handleURIOrFile(const QString &s) {
    ...
    if (QFile::exists(s)) {
        Q_EMIT message(tr("URI handling"), tr("Opening files is not supported."),
                       CClientUIInterface::ICON_WARNING);
        return;
    }
}
