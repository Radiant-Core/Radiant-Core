...
[[maybe_unused]] void CloseLegacyNotification() {
...
[[maybe_unused]] void ConfirmLegacyAddressConvert() {
    QTimer::singleShot(0, Qt::PreciseTimer, []() {
        for (QWidget *widget : QApplication::topLevelWidgets()) {
...
[[maybe_unused]] void ConfirmLegacyAddressUse() {
    QTimer::singleShot(0, Qt::PreciseTimer, []() {
        for (QWidget *widget : QApplication::topLevelWidgets()) {
...
QString NewAddress([[maybe_unused]] bool legacy=false, bool p2sh=false) {
    auto destination = p2sh ?
        CTxDestination(CScriptID()) : CTxDestination(CKeyID());
    std::string destinationString = EncodeLegacyAddr(destination, Params());
...
void TestGUI() {
    ...
    for (QWidget *widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            const QString expected_uri_prefix =
                QString("URI: radiant:");
            QVERIFY(paymentTextList.at(1).indexOf(expected_uri_prefix) != -1);
    ...
    // Legacy-address permission prompts are cashaddr-era behavior and are not
    // applicable after cashaddr removal.
}
