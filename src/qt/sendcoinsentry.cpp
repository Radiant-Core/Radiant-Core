...
SendCoinsEntry::SendCoinsEntry(const PlatformStyle *_platformStyle,
    ...
    // normal Radiant address field
    GUIUtil::setupAddressWidget(ui->payTo, this);
    // just a label for displaying Radiant address(es)
...
void SendCoinsEntry::setModel(WalletModel *_model) {
    ...
    if (_model) {
        ui->messageTextLabel->setToolTip(
            tr("A message that was attached to the radiant: URI which will be stored "
               "with the transaction for your reference. Note: This message "
               "will not be sent over the Radiant network."));
    }
...
void SendCoinsEntry::clear() {
    ...
    // update the display unit, to not use the default ("RXD")
    updateDisplayUnit();
}
