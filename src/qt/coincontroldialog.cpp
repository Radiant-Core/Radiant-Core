...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog) {
    ...
    int nDisplayUnit = BitcoinUnits::RXD;
...
void CoinControlDialog::updateView() {
    ...
    for (const auto &coins : model->wallet().listCoins()) {
        ...
        CTxDestination dest = coins.first;
        QString sWalletAddress = QString::fromStdString(EncodeLegacyAddr(dest, Params()));
        ...
        for (const auto &outpair : coins.second) {
            ...
            if (ExtractDestination(out.txout.scriptPubKey, outputAddress)) {
                sAddress = QString::fromStdString(
                    EncodeLegacyAddr(outputAddress, model->getChainParams()));
