...
std::string DummyAddress(const CChainParams &params) {
    ...
    return MakeAddrInvalid(EncodeLegacyAddr(dstKey, params), params);
}
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent) {
    ...
    widget->setValidator(
        new BitcoinAddressEntryValidator(parent));
...
bool parseBitcoinURI(const QString &scheme, const QUrl &uri,
    ...
    for (auto [key, value] : uriQuery.queryItems()) {
        ...
        if (key == "label") {
            ...
            if (!value.isEmpty()) {
                const auto amount = BitcoinUnits::parse(BitcoinUnits::RXD, false, value);
                if (!amount) {
                    return false;
                }
                rv.amount = *amount;
...
QString formatBitcoinURI(const CChainParams &params,
    ...
    QString ret;
    // Use radiant: URI scheme with the address
    if (IsValidDestinationString(info.address.toStdString(), params)) {
        ret = QString("radiant:") + info.address;
    } else {
        ret = info.address;
    }
    int paramCount = 0;
    ...
    if (info.amount != Amount::zero()) {
        ret +=
            QString("?amount=%1")
                .arg(BitcoinUnits::format(BitcoinUnits::RXD, info.amount, false,
